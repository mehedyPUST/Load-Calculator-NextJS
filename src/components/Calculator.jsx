        ...
        <div className="h-[100dvh] md:min-h-screen bg-gradient-to-br from-slate-800 via-slate-900 to-zinc-950 flex items-center justify-center p-0 md:p-4 antialiased overflow-hidden">
            ...
            <div className="w-full h-full md:h-auto max-w-xl bg-white rounded-none md:rounded-2xl shadow-[0_25px_50px_-12px_rgba(0,0,0,0.5)] overflow-hidden flex flex-col justify-start border-0 md:border border-slate-200">
                ...
                <div className="flex flex-col flex-shrink-0">
                    {/* Header Section */}
                    <div className="bg-gradient-to-r from-emerald-700 to-teal-800 text-white px-3 md:px-6 py-2.5 md:py-4 flex flex-row items-center justify-center gap-3 md:gap-4 shadow-md">
                        <img
                        ...
                        <div className="text-left">
                            ...
                            <p className="text-[11px] md:text-sm text-emerald-100 font-semibold opacity-95">33/11 KV Power Substation</p>
                            <p className="text-[9px] md:text-xs mt-0.5 font-bold tracking-widest text-teal-300 uppercase">Load Calculator Dashboard</p>
                        </div>
                    </div>

                    {/* Digital Clock */}
                    <div className="bg-zinc-900 text-emerald-400 text-center py-1.5 md:py-4 text-xs md:text-base font-extrabold tracking-widest font-mono border-b border-zinc-950 uppercase border-t border-zinc-800">
                    ...
                    <div className="p-2 md:p-4 grid grid-cols-2 gap-2 md:gap-4 bg-slate-50 border-b border-slate-200">
                        <div className="relative flex items-center justify-between bg-white px-2.5 md:px-4 py-1.5 md:py-2 rounded-xl shadow-sm border border-slate-200 hover:border-emerald-500 transition-colors group">
                            <label className="text-xs md:text-sm font-bold text-slate-600 tracking-wide mr-1 group-hover:text-emerald-700">
                                BUS-2 (kV)
                            </label>
                            <input
                                type="number"
                                ...
                                className="w-16 md:w-24 h-7.5 md:h-9 bg-slate-50 border border-slate-200 rounded-md md:rounded-lg text-center text-xs md:text-base font-bold text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 focus:bg-white transition-all font-mono"
                            />
                        </div>
                        <div className="relative flex items-center justify-between bg-white px-2.5 md:px-4 py-1.5 md:py-2 rounded-xl shadow-sm border border-slate-200 hover:border-emerald-500 transition-colors group">
                            <label className="text-xs md:text-sm font-bold text-slate-600 tracking-wide mr-1 group-hover:text-emerald-700">
                ...
                {/* Middle Section: Table Content (ফন্ট সাইজ বাড়িয়ে text-sm করা হয়েছে এবং ইনপুট হাইট একটু রি-ব্যালেন্স করা হয়েছে) */}
                        ...
                        <table className="w-full table-fixed border-collapse">
                            <thead className="sticky top-0 z-10 bg-slate-100 shadow-sm">
                                <tr className="border-b border-slate-200">
                                    <th className="w-[33%] py-1.5 md:py-2 text-center font-bold text-slate-700 text-xs md:text-sm tracking-wider uppercase">LOAD (AMPS)</th>
                                    <th className="w-[34%] py-1.5 md:py-2 text-center font-bold text-slate-700 text-xs md:text-sm tracking-wider uppercase border-x border-slate-200">FEEDER</th>
                                    <th className="w-[33%] py-1.5 md:py-2 text-center font-bold text-slate-700 text-xs md:text-sm tracking-wider uppercase">LOAD (MW)</th>
                                </tr>
                            </thead>
                                ...
                                {tableData.map((item, index) => {
                                    ...
                                    return (
                                        <tr key={item.id} className={`${index % 2 === 0 ? "bg-white" : "bg-slate-50/50"} hover:bg-emerald-50/20 transition-colors`}>
                                            ...
                                            <td className="text-center font-bold text-slate-700 py-1 md:py-1.5 truncate px-1 md:px-3 border-r border-slate-200 text-sm md:text-base">
                                                {item.name}
                                            </td>
                                            <td className="text-center font-black text-emerald-700 py-1 md:py-1.5 font-mono text-sm md:text-base">
                ...
                <div className="flex flex-col flex-shrink-0 bg-white mt-2 md:mt-auto">
                    ...
                    <div className="px-2 md:px-4 pb-2 grid grid-cols-2 gap-2 md:gap-4">
                        <button
                            ...
                            className="bg-emerald-600 hover:bg-emerald-700 text-white h-9 md:h-12 rounded-xl font-extrabold text-xs md:text-sm tracking-wide shadow-md active:scale-[0.98] transition-all border border-emerald-600 uppercase"
                        >
                            ⚡ Calculate
                        </button>
                        <button
                            onClick={handleCopyTotal}
                            className="bg-slate-800 hover:bg-slate-900 text-white h-9 md:h-12 rounded-xl font-extrabold text-xs md:text-sm tracking-wide shadow-md active:scale-[0.98] transition-all border border-slate-700 uppercase"
                        >
                            📋 Copy Total
                        </button>
                    </div>
                    ...
                    {/* Footer */}
                    <div className="bg-slate-100 text-slate-500 text-center py-2 md:py-3 text-[9px] md:text-[11px] font-bold tracking-wider border-t border-slate-200 uppercase mt-auto">
